...
class Builder:
    ...
    def build_cmake_project(
        ...
        """构建CMake项目"""
        try:
            # 检查是否有指定的源码子目录（用于 LLVM 等项目）
            source_subdir = config.get("source_subdir")
            if source_subdir:
                actual_source_dir = source_dir / source_subdir
                if not actual_source_dir.exists():
                    logger.error(
                        f"Source subdirectory {source_subdir} not found in {source_dir}"
                    )
                    return False
                build_dir = source_dir / "build"
                build_dir.mkdir(exist_ok=True)
                # 相对于 build 目录的源码路径
                cmake_source_path = f"../{source_subdir}"
            else:
            ...
            if not (actual_source_dir / "CMakeLists.txt").exists():
                logger.error(f"CMakeLists.txt not found in {actual_source_dir}")
                return False

            # 生成CMake配置参数
            cmake_args = self.generate_cmake_args(config)
            cmake_cmd = f"cmake {cmake_source_path} \\\n    {cmake_args}"
            print(f"will run cmake for {package_name} with src {actual_source_dir}")
            print(f"source dir contains {list(actual_source_dir.iterdir())}")
    ...
    def build_package(self, url: str, config: Dict) -> tuple:
        ...
        # 检查是否有源码子目录配置（如 LLVM）
        source_subdir = config.get("source_subdir")
        cmake_file_location = source_dir / "CMakeLists.txt"
        if source_subdir:
            cmake_file_location = source_dir / source_subdir / "CMakeLists.txt"

        if cmake_file_location.exists():
    ...
    def copy_dynamic_libraries(self):
        ...
        # 处理 DYNAMIC_COPY 模式匹配拷贝
        logger.info("Copying files by pattern matching...")
        for pattern in DYNAMIC_COPY:
            ...
            if found_files:
                ...
                # 记录所有相关的文件（包括符号链接）
                all_related_files = {}  # {real_file_path: [link_names]}
                ...
                for src_file in found_files:
                    ...
                    if src_path.is_symlink():
                        # 如果是符号链接，找到实际文件
                        real_file = src_path.resolve()
                        if real_file.exists():
                            real_file_str = str(real_file)
                            if real_file_str not in all_related_files:
                                all_related_files[real_file_str] = []
                            # 记录符号链接的名字
                            all_related_files[real_file_str].append(src_path.name)
                    else:
                        # 如果是实际文件
                        src_file_str = str(src_path)
                        if src_file_str not in all_related_files:
                            all_related_files[src_file_str] = []

                # 现在处理每个实际文件和它的所有符号链接名
                for real_file, link_names in all_related_files.items():
                    real_path = Path(real_file)

                    # 首先拷贝原文件名
                    success, copied_path = self._copy_library_file(
                        real_file, real_path.name, output_lib_dir, is_rename=False
                    )
                    ...
                    for link_name in link_names:
                        if link_name != real_path.name:  # 避免重复拷贝
                            success, copied_path = self._copy_library_file(
                                real_file,
                                link_name,
                                output_lib_dir,
                                is_rename=True,
                                original_name=real_path.name,
                            )
                            if success and copied_path:
                                copied_files.append(copied_path)
                ...
                for lib_dir in system_lib_dirs:
                    ...
                    for real_file in all_related_files.keys():
                        real_path = Path(real_file)
                        # 查找指向这个实际文件的所有符号链接
                        try:
                            for potential_link in Path(lib_dir).iterdir():
                                if potential_link.is_symlink():
                                    try:
                                        if potential_link.resolve() == real_path:
                                            link_name = potential_link.name
                                            # 检查这个链接名是否已经处理过
                                            already_copied = any(
                                                link_name in existing_links
                                                for existing_links in all_related_files.values()
                                            )
                                            if (
                                                not already_copied
                                                and link_name != real_path.name
                                            ):
                                                success, copied_path = (
                                                    self._copy_library_file(
                                                        real_file,
                                                        link_name,
                                                        output_lib_dir,
                                                        is_rename=True,
                                                ...
                                                if success and copied_path:
                                                    copied_files.append(copied_path)
                                                    logger.info(
                                                        f"Added symlink copy: {link_name} -> {real_path.name}"
                                                    )
